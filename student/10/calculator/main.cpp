...
int main()
{
    ...
    while (true) {
        ...
        string cmd_type = str_to_upper(command_to_be_executed);

        Command command;

        if (!get_command(cmd_type, command)) {
...
string str_to_upper(string word)
{
    ...
    for (string::size_type i = 0; i < word.length(); ++i)
        word_uppercase += toupper(word[i], loc);
    return word_uppercase;
}

bool get_command(string str, Command& command)
{
